...
#if defined(NDEBUG)
# error "Boludoscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x165b247159dc7abee26bf662474c75ea603743da63ad0d40fb813d63d798161b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Boludoscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Boludoscoin Signed Message:\n";
...
{
    // Boludoscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Boludoscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Boludoscoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Boludoscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Boludoscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Boludoscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x165b247159dc7abee26bf662474c75ea603743da63ad0d40fb813d63d798161b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Fed Raises Rates and Signals Faster Pace in Coming Years";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0433cb52b1dd46eb315890f23b0554dcdf026686232a1f5cd289f3f72f4dac2ad6c429364d9745c57388e24f67d6732a82489ebdfbffdba917e640ae31b0f7afe1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521688042;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086285142;

        if (fTestNet)
        {
            block.nTime    = 1521688045;
            block.nNonce   = 2086285142;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xb8c88daf5ad3e6e01c6f0ffb97e16a4d8d1497b47997e691a78b8884ace0cbc2"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb2, 0xd6 }; // Boludoscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BoludoscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BoludoscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BoludoscoinMiner(CWallet *pwallet)
{
    printf("BoludoscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("boludoscoin-miner");
    ...
    try { loop {
        ...
        printf("Running BoludoscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BoludoscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BoludoscoinMiner, pwallet));
}
